...
class IVFIndex(BaseVectorIndex):
    ...
    def _build_index(self) -> None:
        ...
        active_vectors = [(i, vector) for i, vector in enumerate(self._vectors) if vector is not None]
        ...
        n_clusters = min(n_clusters, len(vectors))  # Can't have more clusters than vectors
    ...
    def _kmeans_clustering(self, vectors: np.ndarray, n_clusters: int) -> np.ndarray:
        ...
        for iteration in range(self._max_kmeans_iter):
            ...
            for i, vector in enumerate(vectors):
                distances = [self.euclidean_distance(vector, centroid) for centroid in centroids]
    ...
    def _find_nearest_cluster(self, vector: np.ndarray) -> int:
        ...
        distances = [self.euclidean_distance(vector, centroid) for centroid in self._centroids]
    ...
    def _query_index(self, query_vector: np.ndarray, k: int) -> list[tuple[int, float]]:
        ...
        clusters_to_probe = [cluster_id for cluster_id, _ in cluster_distances[:self._nprobe]]
