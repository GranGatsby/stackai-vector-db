...
def validate_non_empty_text(text: str, error_message: str = "Cannot process empty text") -> str:
...
def validate_name_length(name: str, field_name: str = "Name") -> str:
    ...
    if len(stripped) > settings.max_name_length:
        raise ValueError(f"{field_name} cannot exceed {settings.max_name_length} characters")
    return stripped
...
def validate_title_length(title: str, field_name: str = "Title") -> str:
    ...
    if len(stripped) > settings.max_title_length:
        raise ValueError(f"{field_name} cannot exceed {settings.max_title_length} characters")
    return stripped
...
def validate_index_range(start: int, end: int, field_prefix: str = "") -> tuple[int, int]:
