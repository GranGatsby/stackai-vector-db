    ...
    def validate_embedding(cls, v: list[float]) -> list[float]:
        ...
        for i, val in enumerate(v):
            ...
            if not (-1e6 <= val <= 1e6):  # Reasonable bounds for embedding values
                raise ValueError(f"Embedding value at index {i} is out of reasonable range")
...
class SearchHit(BaseModel):
    ...
    metadata: dict[str, Any] | None = Field(
        None,
        description="Optional chunk metadata for additional context"
    )
...
class SearchResult(BaseModel):
    ...
    index_size: int = Field(
        ...,
        ge=0,
        description="Total number of vectors in the searched index"
    )
    embedding_dim: int = Field(
        ...,
        ge=1,
        le=10000,
        description="Dimension of embeddings used in search"
    )
    query_embedding: list[float] | None = Field(
        None,
        description="Query embedding used (optional, may be large)"
    )
