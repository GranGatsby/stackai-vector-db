...
class IndexStatus(BaseModel):
    ...
    built_at: float | None = Field(
        None, 
        description="Timestamp when index was last built (Unix timestamp)"
    )
    version: int = Field(..., ge=0, description="Version number of the current index")
    dirty_count: int = Field(..., ge=0, description="Number of changes since last build")
...
class BuildIndexRequest(BaseModel):
    ...
    algorithm: IndexAlgo | None = Field(
        None,
        description="Index algorithm to use (uses current/default if None)"
    )
...
class BuildIndexResponse(BaseModel):
    ...
    embedding_dim: int = Field(..., ge=1, le=10000, description="Dimension of embeddings")
    ...
    build_duration: float | None = Field(
        None,
        ge=0.0,
        description="Build duration in seconds (if available)"
    )
    ...
    def from_index_status(
        ...
        if status.embedding_dim is None:
            raise ValueError("Cannot create BuildIndexResponse without embedding dimension")
